import { Check, Edit2, Plus, Search, Settings, Trash2, X } from "lucide-react";
...
const DEFAULT_ENGINES: SearchEngine[] = [
  ...
  },
  {
    id: "sys_scoop",
    name: "Scoop",
    url_template: "https://scoop.sh/#/apps?q=%s",
    is_visible: true,
    sort_order: 3,
  ...
  return (
    <div className="w-full max-w-3xl mx-auto mb-10">
      <form
        ...
        className="relative flex items-center bg-[var(--bg-surface)] rounded-full border border-[var(--border-color)] p-2 shadow-lg transition-shadow hover:shadow-xl hover:border-[var(--text-secondary)] focus-within:border-[var(--accent-color)] focus-within:ring-2 focus-within:ring-[var(--accent-color)]/20 z-10"
      >
...
// 设置弹窗组件
function SearchEngineSettingsModal({
  engines,
  onClose,
  onSave,
}: {
  ...
  onClose: () => void;
  onSave: (engines: SearchEngine[]) => void;
}) {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
          {list
            ...
            .map((item, index) => (
              <div
                key={item.id}
                className="flex flex-col p-3 bg-[var(--bg-surface)] rounded-xl border border-[var(--border-color)]"
              >
                {editingId === item.id ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {item.icon ? (
                      ...
                      <div className="w-5 h-5 bg-[var(--border-color)] rounded-full flex items-center justify-center text-[10px]">
                        {item.name[0]}
                      </div>
                    )}
                      ...
                      <label className="flex items-center cursor-pointer">
                        <div className="relative">
                          <input
                            ...
                            onChange={() => toggleVisible(item.id, item.is_visible)}
                          />
                          <div
                            className={`block w-10 h-6 rounded-full transition-colors ${item.is_visible ? "bg-[var(--accent-color)]" : "bg-[var(--border-color)]"}`}
                          ></div>
                          <div
                            className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${item.is_visible ? "translate-x-4" : ""}`}
                          ></div>
                        </div>
