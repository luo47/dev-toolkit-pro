...
const BYLINE_SUPPORTED_STEP_TYPES = [
  "jsonpath",
  "xpath",
  "css",
  "base64-encode",
...
const parseRegexReplaceOptions = (value: string) => {
  const fallback: RegexReplaceOptions = { pattern: "", flags: "g", replacement: "" };
  try {
    if (!value.startsWith("{")) return fallback;
    return { ...fallback, ...(JSON.parse(value) as Partial<RegexReplaceOptions>) };
  } catch {
    return fallback;
  }
};
...
const parseJsonToXmlOptions = (value: string) => {
  const fallback: JsonToXmlOptions = { root: "root", noRoot: false, noHeader: false };
  try {
    if (value.startsWith("{")) {
      return { ...fallback, ...(JSON.parse(value) as Partial<JsonToXmlOptions>) };
    }
    if (value === "__none__") return { ...fallback, root: "", noRoot: true };
    if (value.trim()) return { ...fallback, root: value.trim() };
  } catch {
    return fallback;
  }
  return fallback;
...
const StepHeader = ({
  ...
  onRemove,
  onMove,
}: Pick<StepItemProps, "step" | "index" | "totalSteps" | "onUpdate" | "onRemove" | "onMove">) => (
...
const RegexReplaceEditor = ({ step, onUpdate }: Pick<StepItemProps, "step" | "onUpdate">) => {
  if (step.type !== "regex-replace") return null;

  const options = parseRegexReplaceOptions(step.value);
...
const ValueEditor = ({ step, onUpdate }: Pick<StepItemProps, "step" | "onUpdate">) => {
  if (!["jsonpath", "xpath", "css", "js"].includes(step.type)) return null;
...
const ByLineOption = ({ step, onToggle }: { step: Step; onToggle: (checked: boolean) => void }) => {
  if (!BYLINE_SUPPORTED_STEP_TYPES.includes(step.type)) return null;
    ...
    <div className="mt-3 pt-3 border-t border-[var(--border-color)]/30">
      <label className="flex items-center gap-2 cursor-pointer select-none group/label w-fit">
        <div className="relative flex items-center justify-center">
          <input
            type="checkbox"
            checked={resolveByLineState(step)}
...
const JsonToXmlEditor = ({ step, onUpdate }: Pick<StepItemProps, "step" | "onUpdate">) => {
  if (step.type !== "json-to-xml") return null;

  const options = parseJsonToXmlOptions(step.value);
...
const StepItem: React.FC<StepItemProps> = ({ step, index, totalSteps, error, onUpdate, onRemove, onMove }) => {
  ...
  return (
    ...
    >
      <StepHeader
        ...
        onMove={onMove}
      />
      <RegexReplaceEditor step={step} onUpdate={onUpdate} />
      <ValueEditor step={step} onUpdate={onUpdate} />
      <ByLineOption step={step} onToggle={handleByLineToggle} />
      <JsonToXmlEditor step={step} onUpdate={onUpdate} />
