...
* `OAUTH_CALLBACK_BASE_URL` 表示 GitHub OAuth 回调地址的基础域名。本地前后端分离时，它应指向 Worker 地址。
* 本地开发时前端页面在 `http://localhost:3000`，GitHub OAuth 回调走 `http://localhost:8787/api/auth/github/callback`，登录成功后服务端会再跳回前端首页。
